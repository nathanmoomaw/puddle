...
- **Fix**: the shader's ~14 independent sine/cosine terms (ambient undulation + oil-film thickness in `usePuddleRenderer.js`) were hardcoded GLSL float literals; they're now uniforms (`uAmbF1..4`, `uThickF1..8`, `uThick2F`, `uPatchF`) defaulting to the original values so the live app is visually unchanged. `capture-puddle-video.mjs` retunes each one to the nearest whole multiple of `2π/loopDuration` before capturing, so every term completes an integer number of cycles across the loop — frame 0 and the frame one tick past the end become mathematically identical, no blending, no reversed playback.
- **Duration/fidelity trade-off** (computed, not guessed): matching the current motion closely needs the loop to be long enough for the quantized frequencies to stay well-separated. At ~24-40s several terms collapse onto the same 1-2 speed bands (loses the layered "richer variation" the original terms were tuned for); at ~79-80s, deviations stay mostly under 20% (worst case ~32%) with distinct bands preserved. Landed on **80s** after confirming the shorter options with the user.
- **Two independent, non-shader periodicity bugs found and fixed** via a PSNR frame-diff check (first captured frame vs. one full loop later — initially only 29dB, i.e. a real visible seam):
  1. `.puddle`'s `puddle-grid-drift 20s linear infinite` CSS `@keyframes` animation runs on the browser's real compositor clock, not `performance.now()`/rAF — the virtual-clock patch can't touch it, and it's visible through the semi-transparent oil shader. Fix: `puddlePage.mjs` now injects `*, *::before, *::after { animation-play-state: paused !important; }` alongside the existing chrome-hiding rule, freezing every CSS animation at capture start so they stay static (trivially periodic) for the whole capture.
  2. The idle hue-rotate/saturation color filter (`App.jsx`) has its own sine with a hardcoded 5000ms period, smoothed through an EMA — the loop duration must be a multiple of 5000ms (80000 qualifies; the earlier 79000 test didn't, and that mismatch was most of the 29dB seam) and the EMA needs several full periods of preroll to settle into its periodic steady state before capture starts, not just decay to a constant. Bumped `PREROLL_MS` default from 3000 to 20000 (4 periods) accordingly.
- **Verified**: re-running the PSNR check after both fixes brought the seam from ~29dB average to **~64dB average** — effectively pixel-identical, confirmed via a standalone script that screenshots the frame at `t=preroll` and the frame at `t=preroll+loopDuration` and diffs them.
- Captures aren't persisted anywhere — `captures/` is gitignored scratch output. All prior capture files (2 videos, 2 stills from 2026-07-07/08) were left untouched; a flawed intermediate 79s test render made during this investigation was deleted and superseded by the corrected 80s output before delivery.
...
- **Fix**: `capture-puddle-video.mjs` now renders small (640×640, where headless software rendering can mostly keep up), applies `ffmpeg minterpolate` (motion-compensated, `mi_mode=mci`) to synthesize a smooth constant 30fps from the sparse real frames, then upscales to the final 1080×1080 with `lanczos` in the same pass as the crossfade loop. Verified clean via extracted-frame inspection (no ghosting/warping) — `mpdecimate` isn't a reliable quality proxy for genuinely-interpolated smooth motion, so judged this one visually. Full 30s pipeline takes ~3.5 minutes (interpolation is the expensive step) but stays well under the 90MB budget (~8MB).
- **Also fixed**: `.mobile-splash` ("puddle / tap to enter") wasn't hidden by the shared chrome-hiding stylesheet — invisible at the old 1280px capture width but blocks the whole canvas at the new smaller 640px capture size (`isMobile()` triggers under 768px). Added to `scripts/lib/puddlePage.mjs`'s hide list alongside `.puddle__label`/`ds-overlay`.
