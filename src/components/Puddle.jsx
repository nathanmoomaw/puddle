      ...
      <defs>
        ...
        */}
        <clipPath id="puddle-shape-clip" clipPathUnits="objectBoundingBox">
          <path d="
            M 0.20 0.08
            C 0.38 0.02, 0.60 0.02, 0.78 0.08
            C 0.90 0.14, 0.97 0.30, 0.97 0.50
            C 0.97 0.70, 0.90 0.86, 0.78 0.92
            C 0.60 0.98, 0.38 0.98, 0.20 0.92
            C 0.08 0.86, 0.03 0.70, 0.03 0.50
            C 0.03 0.30, 0.08 0.14, 0.20 0.08
            Z
          " />
        </clipPath>
      </defs>
      ...
      {puddleMarbles?.map(m => (
        <div
          ...
          style={{
            ...
            '--roll-speed': `${(4.5 + ((m.id ?? 0) % 5) * 0.8).toFixed(1)}s`,
            '--roll-delay': `${0.25 + ((m.id ?? 0) % 3) * 0.6}s`,
            '--roll-a': `${-7 - ((m.id ?? 0) % 4) * 2}deg`,
            '--roll-b': `${7 + ((m.id ?? 0) % 3) * 2}deg`,
          }}
          onPointerDown={(e) => {
            e.stopPropagation()
